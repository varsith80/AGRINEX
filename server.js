...
const server = http.createServer(async (req, res) => {
  ...
  if (urlPath.startsWith('/api/')) {
    ...
    if (urlPath === '/api/buyer/demands/reset' && req.method === 'POST') {
      ...
      db.buyer_demands = seedDemands;
      saveDB(db);
      return sendJSON(res, 200, { success: true, count: seedDemands.length, demands: seedDemands });
    }

    if ((urlPath === '/api/buyer/demands' || urlPath.startsWith('/api/buyer/demands/')) && req.method === 'DELETE') {
      if (!db.buyer_demands) db.buyer_demands = [];
      const demandId = urlPath.startsWith('/api/buyer/demands/') 
        ? decodeURIComponent(urlPath.replace('/api/buyer/demands/', ''))
        : (queryParams.get('id') || '');
      const prevCount = db.buyer_demands.length;
      db.buyer_demands = db.buyer_demands.filter(d => d.id !== demandId);
      saveDB(db);
      return sendJSON(res, 200, { success: true, deletedId: demandId, removed: prevCount !== db.buyer_demands.length });
    }
