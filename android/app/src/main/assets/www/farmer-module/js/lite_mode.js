...
(function () {
  ...
  const LITE_I18N = {
    en: {
      toggleSimple: '👴 Simple Farmer Mode',
  ...
  function getCurrentLang() {
    return localStorage.getItem('agrinex_farmer_language') || localStorage.getItem('agrinex_language') || 'en';
  }
  ...
  function updateToggleBtnState(btn) {
    ...
    if (!btn) return;
    const dict = getDict();
    const textSpan = btn.querySelector('#btn-toggle-lite-text');
    if (isLiteMode) {
      if (textSpan) {
        textSpan.textContent = dict.toggleEnterprise ? dict.toggleEnterprise.replace(/^[^\s]+\s*/, '') : 'Enterprise Mode';
      } else {
      ...
      if (textSpan) {
        textSpan.textContent = dict.toggleSimple ? dict.toggleSimple.replace(/^[^\s]+\s*/, '') : 'Simple Farmer Mode';
      } else {
  ...
  function updateFarmerLiteLanguage(lang) {
    ...
    if (micLabel) micLabel.textContent = dict.floatingMicText || 'Speak Command';
    renderFarmerLiteInterface();
  }
