...
(function () {
  ...
  // State Management
  let copilotLang = localStorage.getItem('agrinex_farmer_language') || localStorage.getItem('agrinex_language') || 'en';
  let copilotHistory = [];
  ...
  const COPILOT_UI_TEXT = {
    'en': {
      badge: 'AI Krishi Mitra v2.0',
      advisorBadge: 'Farmer Advisor',
      listenBtn: '🔊 Listen',
      tags: ['📈 Sell vs Hold Signals', '•', '💰 Mandi Price Comparison', '•', '🐛 Pest & Disease Remedies', '•', '🗣️ 10 Indian Languages'],
      placeholder: 'Ask in English, Hindi, Marathi, or any language: e.g. Onion mandi rates, disease cures...',
      sendBtn: '⚡ Ask AI',
      tip: '💡 Tip: Speak in your mother tongue or type any farming query.',
      langIndicator: 'Language: English'
    },
    'hi': {
      badge: 'कृषि मित्र v2.0',
      advisorBadge: 'किसान सलाहकार',
      listenBtn: '🔊 सुनें / Listen',
      tags: ['📈 आज बेचें या रोकें?', '•', '💰 मंडी भाव तुलना', '•', '🐛 कीट व रोग उपचार', '•', '🗣️ 10 भारतीय भाषाएँ'],
      placeholder: 'हिन्दी, मराठी या किसी भी भाषा में पूछें: उदा. प्याज का भाव, रोग उपचार...',
      sendBtn: '⚡ पूछें',
      tip: '💡 सलाह: हिन्दी या अपनी भाषा में बोलकर या लिखकर सवाल पूछें।',
      langIndicator: 'भाषा: हिन्दी'
    },
    'mr': {
      badge: 'कृषी मित्र v2.0',
      advisorBadge: 'शेतकरी सल्लागार',
      listenBtn: '🔊 ऐका / Listen',
      tags: ['📈 आज विका की थांबा?', '•', '💰 बाजार समिती दर तुलना', '•', '🐛 कीड व रोग उपचार', '•', '🗣️ १० भारतीय भाषा'],
      placeholder: 'मराठी, हिन्दी किंवा कोणत्याही भाषेत विचारा: उदा. कांद्याचा भाव, रोग उपाय...',
      sendBtn: '⚡ विचारा',
      tip: '💡 टीप: मराठीत बोला किंवा कोणताही शेती प्रश्न टाईप करा.',
      langIndicator: 'भाषा: मराठी'
    }
  };

  const COPILOT_CHIPS = {
    'en': [
      { text: '🧅 Onion Price & Trends', query: 'What is the current red onion mandi rate and price forecast?' },
      { text: '🍅 Tomato Selling Advice', query: 'Should I sell tomatoes today or hold for higher rates?' },
      { text: '🌾 Soybean Market Forecast', query: 'What is the 7-day soybean price trend?' },
      { text: '🐛 Pest & Disease Advisory', query: 'How to control blight and pest attacks on vegetables?' },
      { text: '🤝 Negotiate Higher Rate', query: 'How can I negotiate a higher bid from corporate buyers?' },
      { text: '🛡️ Bank Escrow Security', query: 'How does AgriNex 100% escrow bank payment guarantee work?' }
    ],
    'hi': [
      { text: '🧅 प्याज भाव व सलाह', query: 'प्याज आज बेचें या रोकें? क्या भाव चल रहा है?' },
      { text: '🍅 टमाटर बिक्री सलाह', query: 'टमाटर का भाव कब बढ़ेगा? क्या आज बेचना ठीक है?' },
      { text: '🌾 सोयाबीन मूल्य अनुमान', query: 'सोयाबीन का 7 दिवसीय भाव अनुमान क्या है?' },
      { text: '🐛 कीट व रोग उपचार', query: 'सब्जियों पर झुलसा और कीट नियंत्रण के उपाय क्या हैं?' },
      { text: '🤝 भाव बढ़ाकर मांगें', query: 'खरीदार से भाव बढ़ाकर कैसे मांगें?' },
      { text: '🛡️ बैंक एस्क्रो सुरक्षा', query: 'एग्रीनेक्स बैंक एस्क्रो भुगतान कैसे सुरक्षित करता है?' }
    ],
    'mr': [
      { text: '🧅 कांदा भाव व सल्ला', query: 'कांदा आज विकू का? काय भाव चालू आहे?' },
      { text: '🍅 टोमॅटो विक्री सल्ला', query: 'टोमॅटोचा भाव कधी वाढेल? आज विकणे योग्य आहे का?' },
      { text: '🌾 सोयाबीन अंदाज', query: 'सोयाबीन साठवणूक करावी का? पुढील अंदाज काय?' },
      { text: '🐛 कीड व रोग उपाय', query: 'पिकावरील करपा व अळी नियंत्रण उपाय काय आहेत?' },
      { text: '🤝 भाव वाढवून मागा', query: 'खरेदीदाराशी भाव वाढवून कसा मागावा?' },
      { text: '🛡️ बँक पेमेंट सुरक्षा', query: 'बँक एस्क्रो पेमेंट सुरक्षा कशी काम करते?' }
    ]
  };

  function getCopilotUIText(l) {
    return COPILOT_UI_TEXT[l] || COPILOT_UI_TEXT['en'];
  }

  function getCopilotChips(l) {
    return COPILOT_CHIPS[l] || COPILOT_CHIPS['en'];
  }
  ...
  function renderFarmerCopilotChips() {
    const bar = document.getElementById('copilot-chips-bar');
    if (!bar) return;
    const chips = getCopilotChips(copilotLang);
    bar.innerHTML = chips.map(ch => `<button class="copilot-chip" onclick="askFarmerCopilot('${ch.query.replace(/'/g, "\\'")}')">${ch.text}</button>`).join('');
  }

  function injectFarmerCopilotModal() {
    if (document.getElementById('modal-ai-copilot')) return;

    // Ensure copilotLang matches active farmer language
    const currentGlobalLang = (typeof window.getFarmerLanguage === 'function') ? window.getFarmerLanguage() : (localStorage.getItem('agrinex_farmer_language') || localStorage.getItem('agrinex_language') || 'en');
    copilotLang = currentGlobalLang;

    const ui = getCopilotUIText(copilotLang);
    ...
    if (!document.getElementById('btn-floating-copilot')) {
      ...
      floatBtn.title = 'Open AGRIFLEX Krishi AI';
      floatBtn.innerHTML = `
        <span>🤖</span>
    ...
    const langOptionsHTML = langOpts.map(o => `<option value="${o.code}" ${o.code === copilotLang ? 'selected' : ''}>${o.label}</option>`).join('');
      ...
      <div class="modal-overlay" id="modal-ai-copilot">
        <div class="modal-content" style="max-width: 580px; width: 94%; max-height: 88vh; display: flex; flex-direction: column; padding: 18px; border-radius: 20px; box-shadow: 0 20px 60px rgba(0,0,0,0.25); border: 2px solid #0c5a36;">
            ...
            <div style="display: flex; align-items: center; gap: 10px;">
              ...
              <div>
                <div style="display: flex; align-items: center; gap: 6px;">
                  ...
                  <span class="badge" id="copilot-header-badge" style="background: #e0f2fe; color: #0369a1; border: 1px solid #bae6fd; font-size: 0.65rem; font-weight: 800; padding: 1px 6px;">${ui.badge}</span>
                </div>
          ...
          <form onsubmit="handleFarmerCopilotSubmit(event)" style="display: flex; gap: 8px; align-items: center;">
            <button type="button" id="btn-copilot-mic" class="btn-mic-record" onclick="toggleFarmerCopilotSpeechRecognition()" title="Speak in your language" style="width: 42px; height: 42px; min-width: 42px; font-size: 1.15rem; border-radius: 10px;">
              🎤
            </button>
            <input type="text" id="copilot-query-input" class="form-input" placeholder="${ui.placeholder}" style="flex: 1; font-size: 0.88rem; padding: 9px 12px; border-radius: 10px;" />
          ...
          <div id="copilot-mic-status" style="font-size: 0.72rem; color: #64748b; margin-top: 6px; min-height: 16px; display: flex; justify-content: space-between; align-items: center;">
            <span id="copilot-status-text">${ui.tip}</span>
            <span id="copilot-voice-lang-indicator" style="font-weight: 800; color: #0c5a36;">${ui.langIndicator}</span>
          </div>
    ...
    renderFarmerCopilotChips();
  }

  function openFarmerCopilotModal() {
    injectFarmerCopilotModal();
    const currentGlobalLang = (typeof window.getFarmerLanguage === 'function') ? window.getFarmerLanguage() : (localStorage.getItem('agrinex_farmer_language') || localStorage.getItem('agrinex_language') || 'en');
    setFarmerCopilotLanguage(currentGlobalLang);
  ...
  function setFarmerCopilotLanguage(langCode) {
    ...
    const ui = getCopilotUIText(langCode);

    const badgeEl = document.getElementById('copilot-header-badge');
    if (badgeEl) badgeEl.textContent = ui.badge;

    const ind = document.getElementById('copilot-voice-lang-indicator');
    if (ind) ind.textContent = ui.langIndicator;

    const statusEl = document.getElementById('copilot-status-text');
    if (statusEl) statusEl.textContent = ui.tip;
    ...
    if (sendBtn) sendBtn.innerHTML = ui.sendBtn;
  ...
  function renderFarmerCopilotWelcome() {
    ...
    const tagsHTML = ui.tags.map(t => `<span>${t}</span>`).join(' ');
        ...
        <div class="copilot-card-ai" style="background: linear-gradient(135deg, #f0fdf4 0%, #ffffff 100%); border: 1.5px solid #86efac; padding: 14px; border-radius: 14px;">
          <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 6px;">
            <div style="display: flex; align-items: center; gap: 8px;">
              ...
              <span style="background: #dcfce7; color: #166534; font-size: 0.68rem; font-weight: 800; padding: 1px 6px; border-radius: 999px;">${ui.advisorBadge}</span>
            </div>
  ...
  window.addEventListener('agrinex_language_changed', function (e) {
    const newLang = (e.detail && e.detail.lang) ? e.detail.lang : (localStorage.getItem('agrinex_farmer_language') || 'en');
    if (newLang !== copilotLang) {
      setFarmerCopilotLanguage(newLang);
    }
  });
