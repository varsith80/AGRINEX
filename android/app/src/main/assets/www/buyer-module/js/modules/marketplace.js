...
function setUnitDisplay(unitMode, btnElement) {
  ...
  const btns = document.querySelectorAll('.unit-toggle-pill, .unit-toggle-btn');
  btns.forEach((b) => {
  ...
  if (btnElement) {
    ...
    const target = document.getElementById(`btn-unit-${unitMode}`);
    if (target) {
      target.classList.add('active');
      target.style.background = '#0c5a36';
...
function renderVerifiedLots(lotsToRender = null) {
  ...
  if (grid) {
    if (lots.length === 0) {
      ...
      grid.innerHTML = lots
        .map((lot) => {
          const kgRateNum = parseFloat(lot.pricePerKg) || (lot.priceNum ? lot.priceNum / 100 : 0);
          const kgRate = kgRateNum % 1 === 0 ? kgRateNum.toString() : kgRateNum.toFixed(2);
          const qtRateNum = Math.round(kgRateNum * 100);
          const availKg = (lot.availableQtyKg !== undefined) ? lot.availableQtyKg : (lot.qtyNum ? lot.qtyNum * 100 : 5000);
          const availQt = (availKg / 100).toFixed(0);
          ...
          // Authentic Mandi Rate and Direct Arbitrage Math
          const mandiKgRate = lot.mandiRateNum || parseFloat(String(lot.mandiRate || '').replace(/[^0-9.]/g, '')) || (kgRateNum * 1.14);
          const mandiQtRate = Math.round(mandiKgRate * 100);
          const savingsPct = mandiKgRate > kgRateNum ? Math.round(((mandiKgRate - kgRateNum) / mandiKgRate) * 100) : 0;

          // Unit display formatting for Bottom Overlay & Badges
          let priceDisplayHtml = '';
          let availQtyDisplay = '';

          if (unitMode === 'qt') {
            priceDisplayHtml = `₹${qtRateNum.toLocaleString('en-IN')} <span>/ Qt</span>`;
            availQtyDisplay = `${availQt} Qt`;
          } else if (unitMode === 'both') {
            priceDisplayHtml = `₹${kgRate} <span>/ kg</span> <span style="font-size:0.75rem; font-weight:700; opacity:0.92; margin-left:3px;">(₹${qtRateNum.toLocaleString('en-IN')}/Qt)</span>`;
            availQtyDisplay = `${availKg.toLocaleString('en-IN')} kg (${availQt} Qt)`;
          } else {
            // default 'kg'
            priceDisplayHtml = `₹${kgRate} <span>/ kg</span>`;
            availQtyDisplay = `${availKg.toLocaleString('en-IN')} kg`;
          }

          if (isSoldOut) {
            if (isViewingAsOther) {
              availQtyDisplay = `<span style="color:#fca5a5; font-weight:800;">0 ${unitMode === 'qt' ? 'Qt' : 'kg'} (${soldOutText})</span>`;
            } else if (isOwnedByMe) {
            ...
            <div class="farm-lot-card" id="card-${lot.id}" style="${isSoldOut && isViewingAsOther ? 'opacity: 0.94; border: 1.5px solid #fecdd3;' : ''}">
              ...
              <div class="lot-card-body">
                <div>
                  ...
                  <!-- Real-time Mandi Benchmark Comparison & Direct Farm Savings -->
                  <div class="lot-mandi-benchmark-row" style="margin-top: 8px; padding: 6px 10px; background: #f8fafc; border-radius: 8px; border: 1px solid #e2e8f0; display: flex; align-items: center; justify-content: space-between; font-size: 0.74rem;">
                    <div>
                      <span style="color: #64748b; font-weight: 600;">Mandi:</span>
                      <strong style="color: #64748b; text-decoration: line-through; margin-left: 3px;">₹${unitMode === 'qt' ? mandiQtRate.toLocaleString('en-IN') + '/Qt' : mandiKgRate.toFixed(2) + '/kg'}</strong>
                    </div>
                    ${savingsPct > 0 ? `
                      <span class="badge" style="background: #dcfce7; color: #15803d; border: 1px solid #86efac; font-weight: 800; font-size: 0.68rem; padding: 2px 7px; border-radius: 6px;">${savingsPct}% Direct Savings</span>
                    ` : `
                      <span class="badge" style="background: #f1f5f9; color: #475569; font-weight: 700; font-size: 0.68rem; padding: 2px 7px; border-radius: 6px;">Fair Benchmark</span>
                    `}
                  </div>
                </div>
    ...
    if (lots.length === 0) {
      ...
      tbody.innerHTML = lots
        .map((lot) => {
          ...
          const savingsText = savingsPct > 0 ? `${savingsPct}% Lower` : (lot.savings || 'Fair Benchmark');
          ...
          if (unitMode === 'qt') {
            tablePriceHtml = `<strong style="color: #0c5a36; font-size: 0.96rem;">₹ ${qtRateNum.toLocaleString('en-IN')} /Qt</strong>`;
            tableMandiHtml = `Mandi: <span style="text-decoration: line-through;">₹ ${mandiQtRate.toLocaleString('en-IN')} /Qt</span>`;
            qtyColumnHtml = `
              <strong style="font-size: 0.92rem; color: #0f172a;">${availQt} Qt</strong>
              <span style="font-size: 0.72rem; color: #64748b; display: block;">(${availKg.toLocaleString('en-IN')} kg)</span>
            `;
          } else if (unitMode === 'both') {
            tablePriceHtml = `
              <div style="display: flex; flex-direction: column;">
                <strong style="color: #0c5a36; font-size: 0.96rem;">₹ ${kgRate} /kg</strong>
                <span style="font-size: 0.74rem; color: #475569; font-weight: 700;">₹ ${qtRateNum.toLocaleString('en-IN')} /Qt</span>
              </div>
            `;
            tableMandiHtml = `Mandi: <span style="text-decoration: line-through;">₹ ${mandiKgRate.toFixed(2)}/kg (₹ ${mandiQtRate.toLocaleString('en-IN')}/Qt)</span>`;
            qtyColumnHtml = `
              <strong style="font-size: 0.92rem; color: #0f172a;">${availKg.toLocaleString('en-IN')} kg</strong>
              <span style="font-size: 0.72rem; color: #64748b; display: block;">(${availQt} Qt)</span>
            `;
          } else {
            // default 'kg'
            tablePriceHtml = `<strong style="color: #0c5a36; font-size: 0.96rem;">₹ ${kgRate} /kg</strong>`;
            tableMandiHtml = `Mandi: <span style="text-decoration: line-through;">₹ ${mandiKgRate.toFixed(2)} /kg</span>`;
            qtyColumnHtml = `
              <strong style="font-size: 0.92rem; color: #0f172a;">${availKg.toLocaleString('en-IN')} kg</strong>
              <span style="font-size: 0.72rem; color: #64748b; display: block;">(${availQt} Qt)</span>
            `;
          ...
          if (isSoldOut) {
            if (isViewingAsOther || !isOwnedByMe) {
              qtyColumnHtml = `
                <strong style="font-size: 0.92rem; color: #ef4444;">0 ${unitMode === 'qt' ? 'Qt' : 'kg'}</strong>
              ...
              qtyColumnHtml = `
                <strong style="font-size: 0.92rem; color: #15803d;">0 ${unitMode === 'qt' ? 'Qt' : 'kg'}</strong>
