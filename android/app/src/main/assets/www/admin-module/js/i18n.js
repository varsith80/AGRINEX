...
(function () {
  ...
  const SHELL_PHRASES = [
    ...
    ['Cancellation', 'रद्दीकरण', 'रद्द करणे'],
    ...
    ['Dispatches', 'डिस्पैच', 'वाहतूक'],
    ['Dispatch Network', 'डिस्पैच नेटवर्क', 'वाहतूक जाळे'],
    ['Gate Pass', 'गेट पास', 'गेट पास'],
    ['Digital Gate Pass', 'डिजिटल गेट पास', 'डिजिटल गेट पास'],
    ['State-Wide APMC Dispatch Network', 'राज्यव्यापी एपीएमसी डिस्पैच नेटवर्क', 'राज्यव्यापी बाजार समिती वाहतूक जाळे'],
    ['Open Live Driver Console ↗', 'लाइव ड्राइवर कंसोल खोलें ↗', 'थेट चालक कन्सोल उघडा ↗'],
    ['Open Live Driver Console', 'लाइव ड्राइवर कंसोल खोलें', 'थेट चालक कन्सोल उघडा']
  ];
  ...
  let SORTED_RAW_PHRASES = [];
  let SORTED_CROP_ENTRIES = [];
  let SORTED_MANDI_ENTRIES = [];
  let SORTED_BUYER_ENTRIES = [];
  let SORTED_PERSON_ENTRIES = [];
  let SORTED_WAREHOUSE_ENTRIES = [];
  ...
  function buildReverseDictionaries() {
    ...
    // Pre-sort dictionaries longest-first for safe substring and word matching
    SORTED_RAW_PHRASES = [...RAW_PHRASES].sort((a, b) => b[0].length - a[0].length);
    SORTED_CROP_ENTRIES = Object.entries(CROP_MAP).sort((a, b) => b[0].length - a[0].length);
    SORTED_MANDI_ENTRIES = Object.entries(MANDI_MAP).sort((a, b) => b[0].length - a[0].length);
    SORTED_BUYER_ENTRIES = Object.entries(BUYER_MAP).sort((a, b) => b[0].length - a[0].length);
    SORTED_PERSON_ENTRIES = Object.entries(PERSON_MAP).sort((a, b) => b[0].length - a[0].length);
    SORTED_WAREHOUSE_ENTRIES = Object.entries(WAREHOUSE_MAP).sort((a, b) => b[0].length - a[0].length);
  }
  ...
  function tText(text, targetLang) {
    ...
    // Substring replacements for composite phrases using safe word boundaries
    let translated = englishText.trim();
    for (let i = 0; i < SORTED_RAW_PHRASES.length; i++) {
      const row = SORTED_RAW_PHRASES[i];
      if (row[0].length > 3 && translated.includes(row[0])) {
        translated = replaceWordSafe(translated, row[0], row[colIdx]);
      }
    }
    for (const [k, v] of SORTED_CROP_ENTRIES) {
      if (translated.includes(k) && v[l]) {
        translated = replaceWordSafe(translated, k, v[l]);
      }
    }
    for (const [k, v] of SORTED_MANDI_ENTRIES) {
      if (translated.includes(k) && v[l]) {
        translated = replaceWordSafe(translated, k, v[l]);
      }
    }
    for (const [k, v] of SORTED_BUYER_ENTRIES) {
      if (translated.includes(k) && v[l]) {
        translated = replaceWordSafe(translated, k, v[l]);
      }
    }
    for (const [k, v] of SORTED_PERSON_ENTRIES) {
      if (translated.includes(k) && v[l]) {
        translated = replaceWordSafe(translated, k, v[l]);
      }
    }
    for (const [k, v] of SORTED_WAREHOUSE_ENTRIES) {
      if (translated.includes(k) && v[l]) {
        translated = replaceWordSafe(translated, k, v[l]);
      }
    }
  ...
  function walkAndTranslateDOM(rootNode, targetLang) {
    ...
    try {
      ...
      nodesToUpdate.forEach(textNode => {
        ...
        let origEnglish = textNode.__agxOrigText;
        ...
        if (!origEnglish) {
          ...
          if (origEnglish) {
            textNode.__agxOrigText = origEnglish;
          }
        }
  ...
  function setAdminLanguage(lang) {
    ...
    try {
      ...
      if (typeof BroadcastChannel !== 'undefined') {
        ...
        bc.postMessage({ lang: lang, source: 'admin' });
      }
    } catch (e) {}
    ...
    if (typeof window.renderAdminDispatches === 'function') {
      try { window.renderAdminDispatches(); } catch (e) {}
    }
  ...
  if (typeof window !== 'undefined') {
    ...
    window.addEventListener('storage', function (e) {
      if (e.key === STORAGE_KEY || e.key === 'agrinex_language' || e.key === 'agrinex_admin_language' || e.key === 'agrinex_farmer_language' || e.key === 'agrinex_buyer_language' || e.key === 'agrinex_logistics_language') {
    ...
    // BroadcastChannel sync across modules and tabs
    try {
      if (typeof BroadcastChannel !== 'undefined') {
        const bc = new BroadcastChannel('agrinex_language_sync');
        bc.onmessage = function (ev) {
          if (ev.data && ev.data.lang && ['en', 'hi', 'mr'].includes(ev.data.lang) && ev.data.lang !== getAdminLanguage()) {
            setAdminLanguage(ev.data.lang);
          }
        };
      }
    } catch (e) {}
