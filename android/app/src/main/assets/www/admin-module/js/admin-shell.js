...
/* =========================================================================
   6. ADMIN PROFILE MODAL & CREDENTIAL CONTROLLER
   ========================================================================= */
function getStoredAdminProfile() {
  try {
    const customProfile = localStorage.getItem('agrinex_admin_profile');
    if (customProfile) return JSON.parse(customProfile);

    const userStr = localStorage.getItem('agrinex_user');
    if (userStr) {
      const user = JSON.parse(userStr);
      return {
        name: user.name || 'Vikram Malhotra',
        role: user.role || 'Platform Operations Lead',
        email: user.email || 'operations@agrinex.in',
        phone: user.phone || '+91 98201 55432',
        agency: user.agency || 'AgriNex Platform Operations & Governance',
        location: user.location || 'AgriNex Operations HQ, Mumbai, Maharashtra',
        mandiJurisdiction: user.mandiJurisdiction || '305 APMC Mandis across 36 Districts'
      };
    }
  } catch(e) {}

  if (typeof ADMIN_GOVERNANCE_DATA !== 'undefined' && ADMIN_GOVERNANCE_DATA.profile) {
...
function syncAdminProfileUI() {
  ...
  // Update profile name across all pages
  document.querySelectorAll('.profile-name').forEach(el => {
    el.textContent = profile.name;
  });

  // Update profile role if not page-specific
  document.querySelectorAll('.profile-role').forEach(el => {
    el.textContent = profile.role;
  });

  // Update executive status bar elements
  const execName = document.querySelector('.executive-name');
  if (execName) execName.textContent = profile.name;
  const execSub = document.querySelector('.executive-duty-sub');
  if (execSub) execSub.textContent = `${profile.role} • Multi-Sig Signer (Tier-1)`;

  // Update modal preview display elements
  const dispName = document.getElementById('admin-display-name');
  if (dispName) dispName.textContent = profile.name;
  const dispRole = document.getElementById('admin-display-role');
  if (dispRole) dispRole.textContent = `${profile.role} • ${profile.agency || 'AgriNex Platform Operations & Governance'}`;
}

function openAdminProfileModal() {
  const modal = document.getElementById('modal-admin-profile');
  if (!modal) {
    console.warn('modal-admin-profile not found in DOM');
    return;
  }
  ...
  if (nameInput) nameInput.value = profile.name || 'Vikram Malhotra';

  const roleInput = document.getElementById('admin-input-role');
  if (roleInput) roleInput.value = profile.role || 'Platform Operations Lead';

  const emailInput = document.getElementById('admin-input-email');
  if (emailInput) emailInput.value = profile.email || 'operations@agrinex.in';

  const phoneInput = document.getElementById('admin-input-phone');
  if (phoneInput) phoneInput.value = profile.phone || '+91 98201 55432';

  const deptInput = document.getElementById('admin-input-department');
  if (deptInput) deptInput.value = profile.agency || 'AgriNex Platform Operations & Governance';

  const hqInput = document.getElementById('admin-input-hq');
  if (hqInput) hqInput.value = profile.location || 'AgriNex Operations HQ, Nariman Point, Mumbai';

  const jurInput = document.getElementById('admin-input-jurisdiction');
  if (jurInput) jurInput.value = profile.mandiJurisdiction || '305 APMC Mandis across 36 Districts (Maharashtra State)';
...
function saveAdminProfile(e) {
  ...
  const name = document.getElementById('admin-input-name')?.value || 'Vikram Malhotra';
  const role = document.getElementById('admin-input-role')?.value || 'Platform Operations Lead';
  const email = document.getElementById('admin-input-email')?.value || 'operations@agrinex.in';
  const phone = document.getElementById('admin-input-phone')?.value || '+91 98201 55432';
  const agency = document.getElementById('admin-input-department')?.value || 'AgriNex Platform Operations & Governance';
  const location = document.getElementById('admin-input-hq')?.value || 'AgriNex Operations HQ, Nariman Point, Mumbai';
  const mandiJurisdiction = document.getElementById('admin-input-jurisdiction')?.value || '305 APMC Mandis across 36 Districts (Maharashtra State)';
  ...
  try {
    localStorage.setItem('agrinex_admin_profile', JSON.stringify(profile));
    ...
    console.warn('Could not save admin profile to localStorage:', err);
  }

  if (typeof ADMIN_GOVERNANCE_DATA !== 'undefined' && ADMIN_GOVERNANCE_DATA.profile) {
    ADMIN_GOVERNANCE_DATA.profile.name = name;
    ADMIN_GOVERNANCE_DATA.profile.role = role;
    ADMIN_GOVERNANCE_DATA.profile.agency = agency;
    ADMIN_GOVERNANCE_DATA.profile.location = location;
    ADMIN_GOVERNANCE_DATA.profile.mandiJurisdiction = mandiJurisdiction;
  }

  syncAdminProfileUI();
  closeAdminProfileModal();

  if (window.AgriNexToast && typeof window.AgriNexToast.show === 'function') {
    window.AgriNexToast.show({
      icon: '🛡️',
      title: 'Admin Profile Saved',
      message: `Executive credentials saved for ${name} (${role}).`,
      type: 'success'
    });
  } else {
...
// Expose profile methods to window
window.getStoredAdminProfile = getStoredAdminProfile;
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Delegated click handler for profile widget
  document.addEventListener('click', (e) => {
    const profileWidget = e.target.closest('.profile-widget');
    if (profileWidget) {
